...
interface ComponentSelectorProps {
  ...
  suggestions?: Component[]; // optional, default to empty array
}
...
export default function ComponentSelector({
  ...
  const filteredComponents = components
    .filter((component) => {
      ...
      const matchesPrice =
        component.price >= priceFilter[0] && component.price <= priceFilter[1];
  ...
  // Check if a component is suggested
  const isSuggested = (component: Component) => {
    return (
      suggestions?.some((suggestion) => suggestion.id === component.id) ?? false
    );
  };
                ...
                <Button
                  onClick={() => {
                    ...
                    const randomIndex = Math.floor(
                      Math.random() * validComponents.length
                    );
